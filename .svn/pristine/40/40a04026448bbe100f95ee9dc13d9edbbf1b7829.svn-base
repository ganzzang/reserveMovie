...
<link rel="stylesheet" type="text/css" href="/css/reserveMvButton.css">
		...
		<div class="sub_menu">
			...
			<c:choose>
					...
					<c:when test="${sessionScope.loginBean.memberId != null}">
						...
						<li ><a href="/index.do" style="color:blue">첫 화면으로</a></li>		
						</ul>
						</c:if>
						<c:if test="${sessionScope.loginBean.memberId != 'admin'}">
						<ul>
						...
						<li><a href="/index.do" style="color:gray">첫 화면으로</a></li>			
						</ul>
						</c:if>
					</c:when>
					<c:otherwise>
